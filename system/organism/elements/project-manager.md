...
FEEDS     project-page      · /project-page RENDERS the team-readable page FROM this element's brief.md (plus Trello and the registry row); it decides nothing and NEVER writes the brief back. TWO ARTIFACTS PER JOB, ONE PER SKILL: {path}/brief.md is private to the AI Brain and owned HERE; {STREET}.md in the DFY RECORDS Shared Drive is the published copy and owned THERE. ⚠ A job is therefore NOT "a product of /project-page" — it is a project THIS element created, which /project-page later published; and the `project-page` slug in project-registry.md is the skill's OWN build log on the ai-setup desk, not a job [honor; no hook couples them]
