...
# A WINDOWS PATH IS ABSOLUTE AND DOES NOT BEGIN WITH "/". `G:\Shared drives\AI BRAIN\...\brief.md` is
# exactly as absolute as `/g/Shared drives/...`, and the old test -- a bare `${2#/}` compare -- called
...
#   - THE PERSISTENCE HOOK JOINED IT ONTO THE ARMING CWD (`/c/Repo/G:\...\brief.md`), so the brief read
...
# DELIBERATELY NOT OS-GATED, matching 38c81cf: `G:\x` as a RELATIVE filename on Linux is a name nobody
