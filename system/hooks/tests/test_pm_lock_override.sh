...
# REPRO for the bug this section was written against. `G:\...\brief.md` is absolute and does NOT
...
WSESS=win-test
WKEY="sess-$WSESS"
WIN='G:\Shared drives\AI BRAIN\state\projects\marriott\brief.md'
WIN2='G:\Shared drives\AI BRAIN\state\projects\westgate\brief.md'
...
OUT="$(warm "$WIN" marriott business 2>&1)"; RC=$?
[ "$RC" = 0 ] && ok "a drive-letter arm succeeds (exit 0)" || bad "windows arm exit" "exit=$RC $OUT"
[ "$(wlockslug)" = marriott ] && ok "⭐ AND IT WRITES A LOCK — the bug was that it did not" || bad "NO LOCK on a windows path: window left UNLOCKED" "lock_slug=$(wlockslug)"
[ "$(wlockdoc)" = "$WIN" ] && ok "lock_doc keeps the native spelling verbatim" || bad "lock_doc mangled" "$(wlockdoc)"
[ "$(wpmf locked 2>/dev/null)" = marriott ] && ok "\`locked\` reports marriott, not none" || bad "locked verb" "$(wpmf locked 2>&1)"

# and the lock it now writes must actually REFUSE, or it is a file rather than a guard
OUT="$(warm "$WIN2" westgate business 2>&1)"; RC=$?
[ "$RC" = 3 ] && ok "re-arm onto a different project REFUSED (exit 3)" || bad "windows lock does not refuse" "exit=$RC $OUT"
[ "$(wlockslug)" = marriott ] && ok "and nothing moved" || bad "lock moved" "$(wlockslug)"
...
OUT="$(warm bare-slug-not-a-path westgate business 2>&1)"; RC=$?
...
case "$(warm 'G:no-separator' westgate business 2>&1)" in *MALFORMED*) ok "a drive letter with NO separator is malformed too";; *) bad "G:no-separator accepted as absolute" "";; esac
...
# ⛔ MATCH "/G:", NOT "$SB/G:". The hook resolves against $PWD (or the cwd in the event JSON), which
...
case "$WOUT" in *"/G:"*) bad "the windows path was JOINED onto the arming cwd" "$WOUT";; *) ok "and was NOT joined onto a cwd";; esac
